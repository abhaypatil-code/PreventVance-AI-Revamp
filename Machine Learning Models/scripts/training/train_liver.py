...
DATASET_PATH = "../../data/processed/Liver_Processed.csv"
TARGET_COLUMN = "Has_Liver_Disease"
...
RESULTS_FILE = "liver_results.txt"
...
def split_data(X: pd.DataFrame, y: pd.Series, 
    ...
    print("Applying SMOTETomek to handle class imbalance + noise...")
    resampler = SMOTETomek(random_state=random_state)
    X_train_resampled, y_train_resampled = resampler.fit_resample(X_train, y_train)
    print(f"Training set after SMOTETomek: {len(X_train_resampled)} samples")
    print(f"Class distribution after SMOTETomek: {pd.Series(y_train_resampled).value_counts().to_dict()}")
    ...
    selected_features = X_train_resampled.columns[selector.get_support()].tolist()
    ...
    return X_train_final, X_val_final, X_test_final, y_train_resampled, y_val, y_test
...
def get_models(random_state: int = 42) -> Dict[str, Any]:
    ...
    """
    # Define the base MLP with best parameters found
    base_mlp_params = {
    ...
    models = {
        "MLP Classifier": MLPClassifier(**base_mlp_params),
        "Voting MLP Ensemble": VotingClassifier(
            estimators=[
                ('mlp1', MLPClassifier(**{**base_mlp_params, 'random_state': random_state})),
                ('mlp2', MLPClassifier(**{**base_mlp_params, 'random_state': random_state + 1})),
                ('mlp3', MLPClassifier(**{**base_mlp_params, 'random_state': random_state + 2})),
                ('mlp4', MLPClassifier(**{**base_mlp_params, 'random_state': random_state + 3})),
                ('mlp5', MLPClassifier(**{**base_mlp_params, 'random_state': random_state + 4}))
            ],
            voting='soft'
        )
    }
...
def get_model_hyperparameters(model_name: str) -> Dict[str, Any]:
    ...
    param_grids = {
        ...
        "Voting MLP Ensemble": {} # No tuning for voting ensemble in this phase
    }
...
def find_optimal_threshold(model: Any, X_val: pd.DataFrame, y_val: pd.Series) -> float:
    """Find the optimal probability threshold that maximizes J-Statistic (Sensitivity + Specificity - 1)"""
    ...
    print(f"Optimal Threshold found: {best_threshold:.4f} (Max J-Stat: {j_scores[best_idx]:.4f})")
...
def evaluate_model(model: Any, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
    ...
    # Optimize Threshold if model supports probabilities
    best_threshold = 0.5
    
    # Calculate initial metrics with default threshold
    metrics = {
...
def evaluate_model_with_threshold(model: Any, X: pd.DataFrame, y: pd.Series, threshold: float = 0.5) -> Dict[str, float]:
    ...
    if hasattr(model, 'predict_proba'):
         metrics['AUC-ROC'] = roc_auc_score(y, model.predict_proba(X)[:, 1])
    else:
         metrics['AUC-ROC'] = None
...
def generate_model_plots(model: Any, X_test: pd.DataFrame, y_test: pd.Series,
    ...
    # Generate predictions
    if hasattr(model, 'predict_proba'):
        # Use optimal threshold logic if needed, but for plots we usually stick to defaults or pass threshold
        # However, for consistency with reported metrics, we probably should use the optimized logic or standard plots
        # Standard ROC curve handles all thresholds.
        # Confusion matrix is threshold dependent.
        # For simplicity in this function, we assume default threshold call or we pass optimized preds.
        # But this function takes X_test.
        pass
    
    y_pred = model.predict(X_test) # Default 0.5 threshold for plotting to keep it standard, or we could pipe through optimized threshold
    # Ideally we should use the optimized threshold for the confusion matrix
    
    # Custom fix: Check if we have an optimized threshold in results (GLOBAL HACK or pass it in)
    # Since we can't easily pass it in without changing signature everywhere, let's keep consistent with logic
    # BUT wait, the user wants BETTER CM. So we MUST use the optimized threshold for plotting.
    
    # Let's try to infer if model has probability
    if hasattr(model, 'predict_proba'):
         # Retrieve threshold from results by matching model name?? No, that's messy.
         # Standard practice: Plot standard, but maybe the "Test Results" text file is what matters most.
         # Actually, let's just stick to default for plots unless we want to rebuild `generate_model_plots` signature.
         # Given constraints, I will rely on the TEXT REPORT for the optimized metrics (TN, FN values).
         pass
...
def save_results(results: Dict[str, Dict], filepath: str,
    ...
    with open(filepath, 'w') as f:
        ...
        f.write(f"Optimization:   Threshold Tuning (Goal: Max J-Statistic)\n")
        ...
        for model_name, model_results in results.items():
            ...
            f.write(f"  Optimal Threshold: {model_results.get('optimal_threshold', 0.5):.4f}\n")
            f.write("\n")
...
def main():
    ...
    for model_name, model in models.items():
        ...
        opt_thresh = find_optimal_threshold(trained_model, X_val, y_val)
        results[model_name]['optimal_threshold'] = opt_thresh
    ...
    for model_name in results.keys():
        ...
        opt_thresh = results[model_name]['optimal_threshold']
        
        # Evaluate with Optimized Threshold
        print(f"\nEvaluating {model_name} with Threshold: {opt_thresh:.4f}")
        test_metrics = evaluate_model_with_threshold(trained_model, X_test, y_test, threshold=opt_thresh)
